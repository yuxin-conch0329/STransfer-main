...
label_mapping = {"Epithelial":0, "Fibroblast":1, "Macrophage":2, "SMC":3, "Plasma":4,"Endothelial":5, "CD4T":6, "NK":7, "CD8T":8, "Mast":9, "Monocyte":10,"Neutrophil":11, "DC":12}
base_path = r"C:\E\JSU\BIO\file\STransfer\STrafer\datanew\OV"

print(f"processing：source={slicename_s} | target={slicename_t}")
params = ParamConfig(slicename_s, slicename_t)

adata_s, labels_s, graph_dict_s = preprocess_slice_OV(base_path,slicename_s,label_mapping=label_mapping, device=device)
adata_t, labels_t, graph_dict_t = preprocess_slice_OV(base_path,slicename_t,label_mapping=label_mapping,device=device)

# Initialize source domain encoder and classifier
src_encoder = STransferEncoder(input_dim=adata_s.obsm['process'].shape[1]).to(device)
...
tgt_encoder = STransferEncoder(input_dim=adata_s.obsm['process'].shape[1]).to(device)
...
# Define a color mapping for labels (0–12) for consistent coloring in plots
color_mapping = {
    0: "#5698D3",
    1: "#C2A16C",
    2: "#6F6DAF",
